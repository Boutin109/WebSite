/*
  Kyle Boutin Portfolio - Interactive Scripts
  ==========================================
...
function updateActiveNav() {
    const scrollPosition = window.pageYOffset + (nav ? nav.offsetHeight : 0) + 100;
...
function initScrollReveal() {
    const revealElements = document.querySelectorAll('.service-card, .info-card, .skill-chip, .highlight-card, .about-content p, .pricing-banner');
...
// =====================
if (contactForm) {
    const contactRecipient = 'kyle@kbtechnical.services';
    ...
    contactForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            showFormStatus('Your email client should open. Please send the email to complete your message.', 'success');
            contactForm.reset();
            
        } catch (error) {
            console.error('Contact form error:', error);
            showFormStatus(`There was an error. Please email ${contactRecipient} directly.`, 'error');
        } finally {
...
function initSectionAnimations() {
    const sectionHeaders = document.querySelectorAll('.section-header, .section-tag');
